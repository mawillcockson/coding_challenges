...
addNode graph (startNode, endNodes) = AM.alter graph (insertNode) startNode
  where
...
deleteEdge graph startNode endNode = AM.alter graph (aux) startNode
  where
    aux Nothing = Nothing
    aux (Just [node])
...
deleteNode graph startNode = AM.delete withoutNodeGraph startNode
  where
