...
so you ask both of them if they have a bag of flour you can use.
...
def main(iterations: int = NUMBER_OF_ITERATIONS) -> None:
    ...
    print(__doc__)
    print(f"if this scenario happens {iterations:,} times, and you pick someone at random,")
    ...
    print(
        f"""
but if you don't ask beforehand, {person_A_fraction.numerator:,} times out of {person_A_fraction.denominator:,} ({float(person_A_fraction):.1%}) person A will have flour,
and person B will have it {person_B_fraction.numerator:,} times out of {person_B_fraction.denominator:,} ({float(person_B_fraction):.1%})
...
while person B's statement is true in only {total_valid_person_B_scenarios / iterations:.1%} of the scenarios.
"""
    )
