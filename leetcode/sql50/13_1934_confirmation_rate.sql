...
CREATE TABLE Signups (user_id int PRIMARY key, time_stamp timestamp(0));

CREATE TYPE ActionType AS ENUM ('confirmed', 'timeout');

CREATE TABLE Confirmations (
  user_id int NOT NULL REFERENCES Signups (user_id),
  time_stamp timestamp(0) NOT NULL,
  action ActionType NOT NULL,
  CONSTRAINT confirmations_pk PRIMARY key (user_id, time_stamp)
);
...
WITH
    ...
    SELECT
      ...
      count(
        CASE
          ...
          ELSE NULL
        END
      ) OVER w AS confirmations,
      count(Confirmations.action) OVER w AS requests
    FROM
      Signups
  ...
  grouped AS (
    SELECT
      user_id,
      counted.confirmations AS confirmations,
      requests
    FROM
      counted
    GROUP BY
...
SELECT
  ...
  CASE
    ...
    ELSE round(
      cast(
        cast(grouped.confirmations AS double PRECISION) / requests AS numeric
      ),
      2
    )
