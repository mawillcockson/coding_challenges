...
// to the elegance of Zig, and that's a pity. What if someone takes a short,
// first look at the homepage and doesn't get 'enchanted'? The present example
// is simply better suited. It also helps demonstrate the basic principles of
// tokenization, and so we will use it as our introduction to this topic.
//
// In the following exercises we will also read and process data from large
// files. Hopefully by the end it will be clear how useful all this is.
//
...
//    // which transforms numbers as written in a string into their respective
//    // integer values.
...
//    test "parse integers" {
//
//        // A string containing four numbers is created.
...
//
//        // Make sure the memory we use is deallocated at the end of the program's execution. Otherwise the compiler might grumble!
//        defer list.deinit();
//
//        // Now it gets exciting:
//        // A standard tokenizer is called (Zig has several) and
//        // used to locate the positions of the respective separators
//        // (as we saw earlier, these are space and comma) and pass them to an iterator.
//        var it = std.mem.tokenizeAny(u8, input, " ,");
//
//        // The iterator can now be processed in a loop and the
//        // individual numbers can be transformed.
//        while (it.next()) |num| {
//            // But be careful: The numbers are still only available
//            // as strings. This is where the integer parser comes
//            // into play, converting them from their string representation into real integer values.
...
//    }
//
// And thus the example from the homepage.
// Let's summarize the basic steps again:
//
// - We have pieces of data in sequential order, separated from each other
//   with various characters.
//
// - For further processing (e.g. in an array) this data must be
